...
ringbuf_t ringbuf_empty(void)
{
	ringbuf_t out = RC_RINGBUF_INITIALIZER;
	return out;
}

int ringbuf_alloc(ringbuf_t *buf, int size)
{
	// sanity checks
	if (unlikely(buf == NULL))
	{
		cli_printf("ERROR in ringbuf_alloc, received NULL pointer\n");
		return -1;
	}
	if (unlikely(size < 2))
	{
		cli_printf("ERROR in ringbuf_alloc, size must be >=2\n");
		return -1;
	}
	...
	if (buf->d == NULL)
	{
		cli_printf("ERROR in ringbuf_alloc, failed to allocate memory\n");
		return -1;
	}
...
int ringbuf_free(ringbuf_t *buf)
{
	ringbuf_t new = RC_RINGBUF_INITIALIZER;
	if (unlikely(buf == NULL))
	{
		cli_printf("ERROR in ringbuf_free, received NULL pointer\n");
		return -1;
	}
	if (buf->initialized)
		free(buf->d);
...
int ringbuf_reset(ringbuf_t *buf)
{
	...
	if (unlikely(buf == NULL))
	{
		cli_printf("ERROR in ringbuf_reset, received NULL pointer\n");
		return -1;
	}
	if (unlikely(!buf->initialized))
	{
		cli_printf("ERROR ringbuf_reset, ringbuf uninitialized\n");
		return -1;
	}
...
int ringbuf_insert(ringbuf_t *buf, double val)
{
	int new_index;
	// sanity checks
	if (unlikely(buf == NULL))
	{
		cli_printf("ERROR in ringbuf_insert, received NULL pointer\n");
		return -1;
	}
	if (unlikely(!buf->initialized))
	{
		cli_printf("ERROR in ringbuf_insert, ringbuf uninitialized\n");
		return -1;
	}
...
double ringbuf_get_value(ringbuf_t *buf, int pos)
{
	int return_index;
	// sanity checks
	if (unlikely(buf == NULL))
	{
		cli_printf("ERROR in ringbuf_get_value, received NULL pointer\n");
		return -1.0f;
	}
	if (unlikely(pos < 0 || pos > buf->size - 1))
	{
		cli_printf("ERROR in ringbuf_get_value, position out of bounds\n");
		return -1.0f;
	}
	if (unlikely(!buf->initialized))
	{
		cli_printf("ERROR in ringbuf_get_value, ringbuf uninitialized\n");
		return -1.0f;
	}
...
double ringbuf_std_dev(ringbuf_t buf)
{
	int i;
	...
	if (unlikely(!buf.initialized))
	{
		cli_printf("ERROR in ringbuf_std_dev, ringbuf not initialized yet\n");
		return -1.0f;
	}
