...
static mpu_data_t *data_ptr;
static int imu_shutdown_flag = 0;
static filter_t low_pass, high_pass; // for magnetometer Yaw filtering
...
static int __set_gyro_fsr(mpu_gyro_fsr_t fsr, mpu_data_t *data);
static int __set_accel_fsr(mpu_accel_fsr_t, mpu_data_t *data);
static int __set_gyro_dlpf(mpu_gyro_dlpf_t dlpf);
static int __set_accel_dlpf(mpu_accel_dlpf_t dlpf);
...
int __read_dmp_fifo(mpu_data_t *data);
static int __data_fusion(mpu_data_t *data);
static int __mag_correct_orientation(double mag_vec[3]);

void mpu_interrupt_handler(mpu_data_t *p_mpu_data, mpu_config_t mpu_config)
{
	...
	int mag_div_step = mpu_config.mag_sample_rate_div;

	// read data
	ret = __read_dmp_fifo(p_mpu_data);
	...
	if (mpu_config.enable_magnetometer && !mpu_config.read_mag_after_callback)
	{
		if (mag_div_step >= mpu_config.mag_sample_rate_div)
		{
#ifdef DEBUG
			cli_printf("reading mag before callback\n");
#endif
			mpu_read_mag(p_mpu_data);
			mag_div_step = 1;
		}
		else
			mag_div_step++;
	...
	if (mpu_config.enable_magnetometer && mpu_config.read_mag_after_callback)
	{
		...
		{
#ifdef DEBUG
			cli_printf("reading mag after ISR\n");
#endif
			mpu_read_mag(p_mpu_data);
...
mpu_config_t mpu_default_config(void)
{
	mpu_config_t conf;
...
int mpu_set_config_to_default(mpu_config_t *conf)
{
	*conf = mpu_default_config();
	return 0;
}

int mpu_initialize(mpu_data_t *data, mpu_config_t conf)
{
	...
	if (__reset_mpu() < 0)
	{
		cli_printf("ERROR: failed to reset_mpu9250\n");
	...
	{
		cli_printf("ERROR: failed to load gyro calibration offsets\n");

		return -1;
	}
	if (__load_accel_calibration() < 0)
	{
		cli_printf("ERROR: failed to load accel calibration offsets\n");
	...
	if (i2c_write_byte(config.i2c_bus, SMPLRT_DIV, 0x00))
	{
		cli_printf("I2C bus write error\n");
	...
	{
		cli_printf("failed to set gyro fsr\n");

		return -1;
	}
	if (__set_accel_fsr(conf.accel_fsr, data))
	{
		cli_printf("failed to set accel fsr\n");

		return -1;
	}
	if (__set_gyro_dlpf(conf.gyro_dlpf))
	{
		cli_printf("failed to set gyro dlpf\n");

		return -1;
	}
	if (__set_accel_dlpf(conf.accel_dlpf))
	{
		cli_printf("failed to set accel_dlpf\n");
	...
	if (conf.enable_magnetometer)
	{
		...
		{
			cli_printf("failed to initialize magnetometer\n");
...
int mpu_read_accel(mpu_data_t *data)
{
	// new register data stored here
	uint8_t raw[6];
	// Read the six raw data registers into data array
	if (i2c_read_bytes(config.i2c_bus, ACCEL_XOUT_H, 6, &raw[0]) < 0)
	{
		return -1;
	}
...
int mpu_read_gyro(mpu_data_t *data)
{
	// new register data stored here
	uint8_t raw[6];
	// Read the six raw data registers into data array
	if (i2c_read_bytes(config.i2c_bus, GYRO_XOUT_H, 6, &raw[0]) < 0)
	{
		return -1;
	}
	// Turn the MSB and LSB into a signed 16-bit value
...
int mpu_read_mag(mpu_data_t *data)
{
	...
	{
		cli_printf("ERROR: can't read magnetometer unless it is enabled in \n");
		cli_printf("mpu_config_t struct before calling mpu_initialize\n");
		return -1;
	}
	// magnetometer is actually a separate device with its
	// own address inside the mpu9250
	// MPU9250 was put into passthrough mode
	config.i2c_bus = AK8963_ADDR;
...
int mpu_read_temp(mpu_data_t *data)
{
	uint16_t adc;
	// Read the two raw data registers
	if (i2c_read_word(config.i2c_bus, TEMP_OUT_H, &adc) < 0)
	{
		cli_printf("failed to read IMU temperature registers\n");
		return -1;
	}
...
int __reset_mpu(void)
{
	...
	// write the reset bit
	if (i2c_write_byte(config.i2c_bus, PWR_MGMT_1, H_RESET) == -1)
	{
		// wait and try again
		usleep(10000);
		if (i2c_write_byte(config.i2c_bus, PWR_MGMT_1, H_RESET) == -1)
...
int __check_who_am_i(void)
{
	...
	config.i2c_bus = config.i2c_addr;
	if (i2c_read_byte(config.i2c_bus, WHO_AM_I_MPU9250, &c) < 0)
	{
		cli_printf("i2c_read_byte failed reading who_am_i register\n");
		return -1;
	}
	...
	{
		cli_printf("invalid who_am_i register: 0x%x\n", c);
		cli_printf("expected 0x68 or 0x69 for mpu6050/9150, 0x70 for mpu6500, 0x71 for mpu9250, 0x75 for mpu9255,\n");
		return -1;
	}
	return 0;
}

int __set_accel_fsr(mpu_accel_fsr_t fsr, mpu_data_t *data)
{
	uint8_t c;
	switch (fsr)
	{
	...
	case ACCEL_FSR_16G:
		...
		break;
	default:
		cli_printf("invalid accel fsr\n");
		return -1;
	}
	return i2c_write_byte(config.i2c_bus, ACCEL_CONFIG, c);
}

int __set_gyro_fsr(mpu_gyro_fsr_t fsr, mpu_data_t *data)
{
	uint8_t c;
	switch (fsr)
	{
	...
	case GYRO_FSR_2000DPS:
		...
		break;
	default:
		cli_printf("invalid gyro fsr\n");
		return -1;
	}
	return i2c_write_byte(config.i2c_bus, GYRO_CONFIG, c);
}

int __set_accel_dlpf(mpu_accel_dlpf_t dlpf)
{
	...
	case ACCEL_DLPF_5:
		...
		break;
	default:
		cli_printf("invalid config.accel_dlpf\n");
		return -1;
	}
	return i2c_write_byte(config.i2c_bus, ACCEL_CONFIG_2, c);
}

int __set_gyro_dlpf(mpu_gyro_dlpf_t dlpf)
{
	...
	case GYRO_DLPF_5:
		...
		break;
	default:
		cli_printf("invalid gyro_dlpf\n");
		return -1;
	}
	return i2c_write_byte(config.i2c_bus, CONFIG, c);
}
...
int __init_magnetometer(int cal_mode)
{
	...
	if (i2c_write_byte(config.i2c_bus, AK8963_CNTL, MAG_POWER_DN) < 0)
	{
	...
	if (i2c_write_byte(config.i2c_bus, AK8963_CNTL, MAG_FUSE_ROM))
	{
	...
	if (i2c_write_byte(config.i2c_bus, AK8963_CNTL, MAG_POWER_DN))
	{
	...
	if (i2c_write_byte(config.i2c_bus, AK8963_CNTL, c))
	{
...
int __power_off_magnetometer(void)
{
	...
	if (i2c_write_byte(config.i2c_bus, AK8963_CNTL, MAG_POWER_DN) < 0)
	{
		cli_printf("failed to write to magnetometer\n");
		return -1;
	}
	config.i2c_bus = config.i2c_addr;
	return 0;
...
int mpu_power_off(void)
{
	...
	if (config.enable_magnetometer)
		__power_off_magnetometer();
	// write the reset bit
	if (i2c_write_byte(config.i2c_bus, PWR_MGMT_1, H_RESET))
	{
		// wait and try again
		usleep(1000);
		if (i2c_write_byte(config.i2c_bus, PWR_MGMT_1, H_RESET))
		{
			cli_printf("I2C write to MPU9250 Failed\n");
			return -1;
		}
	}
	// write the sleep bit
	if (i2c_write_byte(config.i2c_bus, PWR_MGMT_1, MPU_SLEEP))
	{
		// wait and try again
		usleep(1000);
		if (i2c_write_byte(config.i2c_bus, PWR_MGMT_1, MPU_SLEEP))
...
int mpu_initialize_dmp(mpu_data_t *data, mpu_config_t conf)
{
	int i;
	uint8_t tmp;
	// range check
	...
	{
		cli_printf("ERROR:dmp_sample_rate must be between %d & %d\n",
	...
	{
		cli_printf("DMP sample rate must be a divisor of 200\n");
		cli_printf("acceptable values: 200,100,50,40,25,20,10,8,5,4 (HZ)\n");
		return -1;
	}
	// make sure the compass filter time constant is valid
	if (conf.enable_magnetometer && conf.compass_time_constant <= 0.1)
	{
		cli_printf("ERROR: compass time constant must be greater than 0.1\n");
		return -1;
	}
	...
	{
		cli_printf("WARNING, gyro dlpf bandwidth must be <= 184hz in DMP mode\n");
		cli_printf("setting to 184hz automatically\n");
		conf.gyro_dlpf = GYRO_DLPF_184;
	}
	if (conf.accel_dlpf == ACCEL_DLPF_OFF || conf.accel_dlpf == ACCEL_DLPF_460)
	{
		cli_printf("WARNING, accel dlpf bandwidth must be <= 184hz in DMP mode\n");
		cli_printf("setting to 184hz automatically\n");
		conf.accel_dlpf = ACCEL_DLPF_184;
	}
	// check FSR
	...
	{
		cli_printf("WARNING, gyro FSR must be GYRO_FSR_2000DPS in DMP mode\n");
		cli_printf("setting to 2000DPS automatically\n");
		config.gyro_fsr = GYRO_FSR_2000DPS;
	}
	if (conf.accel_fsr != ACCEL_FSR_8G)
	{
		cli_printf("WARNING, accel FSR must be ACCEL_FSR_8G in DMP mode\n");
		cli_printf("setting to ACCEL_FSR_8G automatically\n");
	...
	if (__reset_mpu() < 0)
	{
		cli_printf("failed to __reset_mpu()\n");
	...
	if (i2c_write_byte(config.i2c_bus, ACCEL_CONFIG_2, tmp))
	{
		cli_printf("ERROR: in mpu_initialize_dmp, failed to write to ACCEL_CONFIG_2 register\n");
	...
	{
		cli_printf("ERROR in mpu_initialize_dmp, failed to set gyro_fsr register\n");

		return -1;
	}
	if (__set_accel_fsr(config.accel_fsr, data_ptr) == -1)
	{
		cli_printf("ERROR in mpu_initialize_dmp, failed to set accel_fsr register\n");
	...
	if (__mpu_set_sample_rate(400) < 0)
	{
		// if(__mpu_set_sample_rate(config.dmp_sample_rate)<0){
		cli_printf("ERROR: setting IMU sample rate\n");
	...
	if (__mpu_set_bypass(1))
	{
		cli_printf("failed to run __mpu_set_bypass\n");
	...
	{
		cli_printf("failed to load DMP motion driver\n");
	...
	{
		cli_printf("ERROR: failed to set dmp orientation\n");
	...
	{
		cli_printf("ERROR: failed to enable DMP features\n");
	...
	{
		cli_printf("ERROR: failed to set DMP fifo rate\n");
	...
	{
		cli_printf("ERROR: __mpu_set_dmp_state(1) failed\n");
	...
	{
		cli_printf("ERROR: failed to set DMP interrupt mode to continuous\n");
...
int __mpu_write_mem(unsigned short mem_addr, unsigned short length, unsigned char *data)
{
	...
	if (!data)
	{
		cli_printf("ERROR: in mpu_write_mem, NULL pointer\n");
		return -1;
	}
	...
	{
		cli_printf("mpu_write_mem exceeds bank size\n");
		return -1;
	}
	if (i2c_write_bytes(config.i2c_bus, MPU6500_BANK_SEL, 2, tmp))
		return -1;
	if (i2c_write_bytes(config.i2c_bus, MPU6500_MEM_R_W, length, data))
		return -1;
	return 0;
}
...
int __mpu_read_mem(unsigned short mem_addr, unsigned short length, unsigned char *data)
{
	...
	if (tmp[1] + length > MPU6500_BANK_SIZE)
	{
		cli_printf("mpu_read_mem exceeds bank size\n");
		return -1;
	}
	if (i2c_write_bytes(config.i2c_bus, MPU6500_BANK_SEL, 2, tmp))
		return -1;
	if (i2c_read_bytes(config.i2c_bus, MPU6500_MEM_R_W, length, data) != length)
		return -1;
	return 0;
}
...
int __dmp_load_motion_driver_firmware(void)
{
	...
	for (ii = 0; ii < DMP_CODE_SIZE; ii += this_write)
	{
		...
		{
			cli_printf("dmp firmware write failed\n");
			return -1;
		}
		if (__mpu_read_mem(ii, this_write, cur))
		{
			cli_printf("dmp firmware read failed\n");
			return -1;
		}
		if (memcmp(dmp_firmware + ii, cur, this_write))
		{
			cli_printf("dmp firmware write corrupted\n");
			return -2;
		}
	}
	...
	if (i2c_write_bytes(config.i2c_bus, MPU6500_PRGM_START_H, 2, tmp))
	{
		cli_printf("ERROR writing to MPU6500_PRGM_START register\n");
		return -1;
	}
	return 0;
}
...
int __dmp_set_orientation(unsigned short orient)
{
	...
	{
		cli_printf("ERROR: in dmp_set_orientation, failed to write dmp mem\n");
		return -1;
	}
...
int __dmp_set_fifo_rate(unsigned short rate)
{
	...
	div = (unsigned short)DMP_MAX_RATE / rate - 1;
	tmp[0] = (unsigned char)((div >> 8) & 0xFF);
	tmp[1] = (unsigned char)(div & 0xFF);
	if (__mpu_write_mem(D_0_22, 2, tmp))
	{
		cli_printf("ERROR: writing dmp sample rate reg");
		return -1;
	}
	if (__mpu_write_mem(CFG_6, 12, (unsigned char *)regs_end))
	{
		cli_printf("ERROR: writing dmp regs_end");
		return -1;
	}
	return 0;
}
...
int __mpu_set_bypass(uint8_t bypass_on)
{
	...
	if (i2c_write_byte(config.i2c_bus, USER_CTRL, tmp))
	{
	...
	tmp = LATCH_INT_EN | INT_ANYRD_CLEAR | ACTL_ACTIVE_HIGH; // latching
	// tmp =  ACTL_ACTIVE_LOW;	// non-latching
	if (bypass_on)
		tmp |= BYPASS_EN;
	if (i2c_write_byte(config.i2c_bus, INT_PIN_CFG, tmp))
	{
...
int __mpu_reset_fifo(void)
{
	...
	data = 0;
	if (i2c_write_byte(config.i2c_bus, INT_ENABLE, data))
		return -1;
	if (i2c_write_byte(config.i2c_bus, FIFO_EN, data))
		return -1;
	if (i2c_write_byte(config.i2c_bus, USER_CTRL, data))
		return -1;
	...
	if (i2c_write_byte(config.i2c_bus, USER_CTRL, data))
		return -1;
	// usleep(1000); // how I had it
	usleep(50000); // invensense standard
 ...
 * necessary to remain in its current form as mpu_initialize_dmp uses a fixed
...
int __dmp_enable_feature(unsigned short mask)
{
	...
	{
		cli_printf("ERROR: in dmp_enable_feature, failed to write mpu mem\n");
		return -1;
	}
...
int __set_int_enable(unsigned char enable)
{
	...
	if (i2c_write_byte(config.i2c_bus, INT_ENABLE, tmp))
	{
		cli_printf("ERROR: in set_int_enable, failed to write INT_ENABLE register\n");
		return -1;
	}
	// disable all other FIFO features leaving just DMP
	if (i2c_write_byte(config.i2c_bus, FIFO_EN, 0))
	{
		cli_printf("ERROR: in set_int_enable, failed to write FIFO_EN register\n");
		return -1;
	}
	return 0;
}
...
int __mpu_set_sample_rate(int rate)
{
	if (rate > 1000 || rate < 4)
	{
		cli_printf("ERROR: sample rate must be between 4 & 1000\n");
		return -1;
	}
	/* Keep constant sample rate, FIFO rate controlled by DMP. */
	uint8_t div = (1000 / rate) - 1;
#ifdef DEBUG
	cli_printf("setting divider to %d\n", div);
#endif
	if (i2c_write_byte(config.i2c_bus, SMPLRT_DIV, div))
	{
		cli_printf("ERROR: in mpu_set_sample_rate, failed to write SMPLRT_DIV register\n");
		return -1;
	}
	return 0;
}
 ...
 * isn't necessary as mpu_initialize_dmp sets these registers but it remains
...
int __mpu_set_dmp_state(unsigned char enable)
{
	if (enable)
	{
		...
		i2c_write_byte(config.i2c_bus, FIFO_EN, 0);
 ...
 * @return     0 on success, -1 on failure
 */
int mpu_set_dmp_callback(void (*func)(void))
{
	if (func == NULL)
	{
		cli_printf("ERROR: trying to assign NULL pointer to dmp_callback_func\n");
		return -1;
	}
...
int mpu_set_tap_callback(void (*func)(int dir, int cnt))
{
	if (func == NULL)
	{
		cli_printf("ERROR: trying to assign NULL pointer to tap_callback_func\n");
		return -1;
	}
...
int __read_dmp_fifo(mpu_data_t *data)
{
	...
	if (!dmp_en)
	{
		cli_printf("only use mpu_read_fifo in dmp mode\n");
		return -1;
	}
	...
	{
		cli_printf("ERROR: packet_len is set incorrectly for read_dmp_fifo\n");
		return -1;
	}
	int is_new_dmp_data = 0;

	// check fifo count register to make sure new data is there
	if (i2c_read_word(config.i2c_bus, FIFO_COUNTH, &fifo_count) < 0)
	{
		if (config.show_warnings)
		{
			cli_printf("fifo_count i2c error: %s\n", strerror(errno));
		}
		return -1;
	}
#ifdef DEBUG
	cli_printf("fifo_count: %d\n", fifo_count);
#endif
	...
	else if (fifo_count == 2 * packet_len)
	{
		if (config.show_warnings && first_run != 1)
		{
			cli_printf("warning: imu fifo contains two packets\n");
		}
		i = packet_len;
	}
	else if (fifo_count == 3 * packet_len)
	{
		if (config.show_warnings && first_run != 1)
		{
			cli_printf("warning: imu fifo contains three packets\n");
		}
		i = 2 * packet_len;
	}
	else if (fifo_count == 4 * packet_len)
	{
		if (config.show_warnings && first_run != 1)
		{
			cli_printf("warning: imu fifo contains four packets\n");
		}
		i = 2 * packet_len;
	}
	else if (fifo_count == 5 * packet_len)
	{
		if (config.show_warnings && first_run != 1)
		{
			cli_printf("warning: imu fifo contains five packets\n");
		}
		i = 2 * packet_len;
	}
	...
	else
	{
		if (config.show_warnings && first_run != 1)
		{
			cli_printf("warning: %d bytes in FIFO, expected %d\n", fifo_count, packet_len);
		}
		__mpu_reset_fifo();
		return -1;
	}
	...
	// read it in!
	ret = i2c_read_bytes(config.i2c_bus, FIFO_R_W, fifo_count, &raw[0]);
	if (ret < 0)
	{
	...
	if (ret != fifo_count)
	{
		if (config.show_warnings)
		{
			cli_printf("ERROR: failed to read fifo buffer register\n");
			cli_printf("read %d bytes, expected %d\n", ret, packet_len);
		}
		return -1;
	}
	...
	if ((quat_mag_sq < QUAT_MAG_SQ_MIN) || (quat_mag_sq > QUAT_MAG_SQ_MAX))
	{
		if (config.show_warnings)
		{
			cli_printf("warning: Quaternion out of bounds, fifo_count: %d\n", fifo_count);
		}
		__mpu_reset_fifo();
		return -1;
	}
	...
	quaternion_to_tb_array(data->dmp_quat, data->dmp_TaitBryan);
	is_new_dmp_data = 1;
	...
	{
#ifdef DEBUG
		cli_printf("running data_fusion\n");
#endif
		__data_fusion(data);
	}
 ...
 * @return     0 on success, -1 on failure
 */
int __data_fusion(mpu_data_t *data)
{
	...
	quaternion_from_tb_array(tilt_tb, tilt_q);
	...
	quaternion_rotate_vector_array(mag_vec, tilt_q);
	...
	if (first_run)
	{
		...
		filter_first_order_lowpass(&low_pass, dt, config.compass_time_constant);
		filter_first_order_highpass(&high_pass, dt, config.compass_time_constant);
		filter_prefill_inputs(&low_pass, startMagYaw);
		filter_prefill_outputs(&low_pass, startMagYaw);
		filter_prefill_inputs(&high_pass, newDMPYaw);
		filter_prefill_outputs(&high_pass, 0);
		first_run = 0;
	}

	// new Yaw is the sum of low and high pass complementary filters.
	double lp = filter_march(&low_pass, newMagYaw + (TWO_PI * mag_spin_counter));
	double hp = filter_march(&high_pass, newDMPYaw + (TWO_PI * dmp_spin_counter));
	newYaw = lp + hp;
	...
	quaternion_from_tb_array(data->fused_TaitBryan, data->fused_quat);
	return 0;
}
...
int __load_gyro_calibration(void)
{
	...
	if (i2c_write_bytes(config.i2c_bus, XG_OFFSET_H, 6, &data[0]))
	{
...
int __load_accel_calibration(void)
{
	...
	// read factory bias
	if (i2c_read_bytes(config.i2c_bus, XA_OFFSET_H, 2, &raw[0]) < 0)
	{
		return -1;
	}
	if (i2c_read_bytes(config.i2c_bus, YA_OFFSET_H, 2, &raw[2]) < 0)
	{
		return -1;
	}
	if (i2c_read_bytes(config.i2c_bus, ZA_OFFSET_H, 2, &raw[4]) < 0)
	{
		return -1;
	}
	// Turn the MSB and LSB into a signed 16-bit value
	...
	if (i2c_write_bytes(config.i2c_bus, XA_OFFSET_H, 2, &raw[0]) < 0)
	{
		cli_printf("ERROR: failed to write X accel offsets into IMU register\n");
		return -1;
	}
	if (i2c_write_bytes(config.i2c_bus, YA_OFFSET_H, 2, &raw[2]) < 0)
	{
		cli_printf("ERROR: failed to write Y accel offsets into IMU register\n");
		return -1;
	}
	if (i2c_write_bytes(config.i2c_bus, ZA_OFFSET_H, 2, &raw[4]) < 0)
	{
...
int mpu_calibrate_gyro_routine(mpu_config_t conf)
{
	uint8_t c, data[6];
	...
	i2c_write_byte(conf.i2c_bus, PWR_MGMT_1, 0x01);
	i2c_write_byte(conf.i2c_bus, PWR_MGMT_2, 0x00);
	usleep(200000);
	...
	// fcli_printf(stderr,"ERROR: failed to load gyro offsets into IMU register\n");
	// return -1;
	// }

	i2c_write_byte(conf.i2c_bus, INT_ENABLE, 0x00);	  // Disable all interrupts
	i2c_write_byte(conf.i2c_bus, FIFO_EN, 0x00);	  // Disable FIFO
	i2c_write_byte(conf.i2c_bus, PWR_MGMT_1, 0x00);	  // Turn on internal clock source
	i2c_write_byte(conf.i2c_bus, I2C_MST_CTRL, 0x00); // Disable I2C master
	i2c_write_byte(conf.i2c_bus, USER_CTRL, 0x00);	  // Disable FIFO and I2C master
	i2c_write_byte(conf.i2c_bus, USER_CTRL, 0x0C);	  // Reset FIFO and DMP
	usleep(15000);

	// Configure MPU9250 gyro and accelerometer for bias calculation
	i2c_write_byte(conf.i2c_bus, CONFIG, 0x01);		// Set low-pass filter to 188 Hz
	i2c_write_byte(conf.i2c_bus, SMPLRT_DIV, 0x04); // Set sample rate to 200hz
	// Set gyro full-scale to 250 degrees per second, maximum sensitivity
	i2c_write_byte(conf.i2c_bus, GYRO_CONFIG, 0x00);
	// Set accelerometer full-scale to 2 g, maximum sensitivity
	i2c_write_byte(conf.i2c_bus, ACCEL_CONFIG, 0x00);

COLLECT_DATA:

	// Configure FIFO to capture gyro data for bias calculation
	i2c_write_byte(conf.i2c_bus, USER_CTRL, 0x40); // Enable FIFO
	// Enable gyro sensors for FIFO (max size 512 bytes in MPU-9250)
	c = FIFO_GYRO_X_EN | FIFO_GYRO_Y_EN | FIFO_GYRO_Z_EN;
	i2c_write_byte(conf.i2c_bus, FIFO_EN, c);
	// 6 bytes per sample. 200hz. wait 0.4 seconds
	usleep(400000);

	// At end of sample accumulation, turn off FIFO sensor read
	i2c_write_byte(conf.i2c_bus, FIFO_EN, 0x00);
	// read FIFO sample count and log number of samples
	i2c_read_bytes(conf.i2c_bus, FIFO_COUNTH, 2, &data[0]);
	...
	for (i = 0; i < samples; i++)
	{
		...
		if (i2c_read_bytes(conf.i2c_bus, FIFO_R_W, 6, data) < 0)
		{
...
int mpu_calibrate_mag_routine(mpu_config_t conf)
{
	int i;
	double new_scale[3];
...
int __collect_accel_samples(int *avg_raw)
{
	...
	// Configure FIFO to capture gyro data for bias calculation
	i2c_write_byte(config.i2c_bus, USER_CTRL, 0x40); // Enable FIFO
	// Enable accel sensors for FIFO (max size 512 bytes in MPU-9250)
	i2c_write_byte(config.i2c_bus, FIFO_EN, FIFO_ACCEL_EN);
	// 6 bytes per sample. 200hz. wait 0.4 seconds
	usleep(400000);

	// At end of sample accumulation, turn off FIFO sensor read
	i2c_write_byte(config.i2c_bus, FIFO_EN, 0x00);
	// read FIFO sample count and log number of samples
	i2c_read_bytes(config.i2c_bus, FIFO_COUNTH, 2, &data[0]);
	...
	{
		// read data for averaging
		if (i2c_read_bytes(config.i2c_bus, FIFO_R_W, 6, data) < 0)
		{
...
int mpu_calibrate_accel_routine(mpu_config_t conf)
{
	int ret, i, j;
	int avg_raw[6][3];

	// save bus and address globally for other functions to use
	config.i2c_bus = conf.i2c_addr;
	config.i2c_addr = conf.i2c_addr;
	...
	// set up the IMU specifically for calibration.
	i2c_write_byte(config.i2c_addr, PWR_MGMT_1, 0x01);
	i2c_write_byte(config.i2c_addr, PWR_MGMT_2, 0x00);
	usleep(200000);

	i2c_write_byte(config.i2c_bus, INT_ENABLE, 0x00);	// Disable all interrupts
	i2c_write_byte(config.i2c_bus, FIFO_EN, 0x00);		// Disable FIFO
	i2c_write_byte(config.i2c_bus, PWR_MGMT_1, 0x00);	// Turn on internal clock source
	i2c_write_byte(config.i2c_bus, I2C_MST_CTRL, 0x00); // Disable I2C master
	i2c_write_byte(config.i2c_bus, USER_CTRL, 0x00);	// Disable FIFO and I2C master
	i2c_write_byte(config.i2c_bus, USER_CTRL, 0x0C);	// Reset FIFO and DMP
	usleep(15000);

	// Configure MPU9250 gyro and accelerometer for bias calculation
	i2c_write_byte(config.i2c_bus, CONFIG, 0x01);		// Set low-pass filter to 188 Hz
	i2c_write_byte(config.i2c_bus, SMPLRT_DIV, 0x04);	// Set sample rate to 200hz
	i2c_write_byte(config.i2c_bus, GYRO_CONFIG, 0x00);	// set G FSR to 250dps
	i2c_write_byte(config.i2c_bus, ACCEL_CONFIG, 0x00); // set A FSR to 2G
	...
	{
		cli_printf("ERROR: in mpu_calibrate_accel_routine, failed to alloc data matrix\n");
		return -1;
	}
	...
	{
		cli_printf("ERROR in mpu_calibrate_accel_routine, failed to write to disk\n");
		return -1;
	}
	vector_free(&center);
	vector_free(&lengths);
...
int mpu_block_until_dmp_data(void)
{
	if (imu_shutdown_flag != 0)
	{
		cli_printf("ERROR: call to mpu_block_until_dmp_data after shutting down mpu\n");
		return -1;
	}
	if (!thread_running_flag)
	{
		cli_printf("ERROR: call to mpu_block_until_dmp_data when DMP handler not running\n");
		return -1;
	}
...
int mpu_block_until_tap(void)
{
	if (imu_shutdown_flag != 0)
	{
		cli_printf("ERROR: call to mpu_block_until_tap after shutting down mpu\n");
		return -1;
	}
	if (!thread_running_flag)
	{
		cli_printf("ERROR: call to mpu_block_until_tap when DMP handler not running\n");
		return -1;
	}
...
static int __mag_correct_orientation(double mag_vec[3])
{
	...
	case ORIENTATION_X_BACK:
		...
		break;
	default:
		cli_printf("ERROR: reading magnetometer, invalid orientation\n");
		return -1;
	}
	return 0;
}
