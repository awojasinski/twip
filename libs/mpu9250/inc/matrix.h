...
#define RC_MATRIX_INITIALIZER \
	...
	/**
 * @brief      Returns an matrix_t with no allocated memory and the
 * initialized flag set to 0.
 *
 * This is essential for initializing matrix_t structs when they are declared
 ...
 * matrices with this before using matrix_alloc or any other function.
 *
 * @return     Returns an empty matrix_t
 */
 ...
 * until more memory is allocated such as with matrix_alloc or
 * matrix_zeros. Will only fail and return -1 if it is passed a NULL pointer.
 *
 * @param      A     Pointer to user's matrix struct
 *
 * @return     0 on success, -1 on failure.
 ...
 * passing an vector_t struct or matrix_t struct as a pointer when its
 ...
 * leaks. Use matrix_add_inplace if you do not need to keep the contents of
 ...
 *
 * The original contents of A are lost. Use matrix_add if you wish to keep
 ...
 * A are not needed after transposing then use matrix_transpose_inplace
 * instead.
 *
 ...
 *
 * Use as an alternative to matrix_transpose if you no longer have need for
