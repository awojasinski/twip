 ...
 * PID, and integrators.
 *
 * See the test_filter.c example for use case.
 *
 ...
 */
	typedef struct filter_t
	{
		...
		vector_t num; ///< numerator coefficients
		vector_t den; ///< denominator coefficients
		///@}
...
#define RC_FILTER_INITIALIZER              \
	...
	/**
 * @brief      Critical function for initializing filter_t structs.
 *
 * This is a very important function. If your filter_t struct is not a global
 ...
 * initialize your filters with filter_empty before using with any other
 * function in this library such as filter_alloc. This serves the same
 * purpose as matrix_empty, vector_empty, and ringbuf_empty.
 *
 * @return     Empty zero-filled filter_t struct
 */
 ...
 * filter_t pointer f points to an existing filter then the old filter's
 * contents are freed safely to avoid memory leaks. We suggest initializing
 * filter f with filter_empty before calling this function if it is not a
 ...
 *
 * @param[out] f     Pointer to user's filter_t struct
 ...
 */
	...
	/**
 * @brief      Like filter_alloc(), but takes arrays for the numerator and
 ...
 *
 * @param      f     Pointer to user's filter_t struct
 *
 ...
 * to work. This assumes that the user does indeed call filter_march at
 ...
 * a call to filter_reset so the filter will soft-start again after each
 ...
 * in time we suggest creating your own ringbuf_t ring buffer.
 *
 * @param      f      Pointer to user's filter_t struct
 ...
 *
 * @param[in]  f1    Pointer to user's filter_t struct to be multiplied
 * @param[in]  f2    Pointer to user's filter_t struct to be multiplied
 * @param[out] out   Pointer to newly created filter struct
 *
 * @return     Returns 0 on success or -1 on failure.
 */
	int filter_multiply(filter_t f1, filter_t f2, filter_t *out);
	/**
 ...
 * @param[in]  f1    Pointer to user's filter_t struct to be multiplied
 * @param[in]  f2    Pointer to user's filter_t struct to be multiplied
 * @param[in]  f3    Pointer to user's filter_t struct to be multiplied
 * @param[out] out   Pointer to newly created filter struct
 *
 * @return     Returns 0 on success or -1 on failure.
 */
 ...
 * steady-state input. This can be used alongside first_order_highpass to
 ...
 * @return     Returns 0 on success or -1 on failure.
 */
	int filter_first_order_lowpass(filter_t *f, double dt, double tc);
 ...
 * @return     Returns 0 on success or -1 on failure.
 */
	int filter_first_order_highpass(filter_t *f, double dt, double tc);
 ...
 * filter. It is simply copied into the 'dt' field of the filter_t struct.
