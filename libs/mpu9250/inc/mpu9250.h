 ...
 * read directly at any time. To use this mode, call mpu_initialize() with
 * your imu_config and imu_data structs as arguments as defined below. You can
 * then call mpu_read_accel, mpu_read_gyro, mpu_read_mag, or
 * mpu_read_temp at any time to get the latest sensor values.
 *
 * ##DMP Mode
 *
 ...
 * function of your choosing set with the mpu_set_dmp_callback() function.
 *
...
#define G_TO_MS2 9.80665		   ///< multiply to convert G to m/s^2, standard gravity definition
	 ...
	 */
	typedef enum mpu_orientation_t
	{
	 ...
	 *
	 * Configuration struct passed to mpu_initialize and mpu_initialize_dmp.
	 * It is best to get the default config with mpu_default_config() function
	...
	typedef struct mpu_config_t
	{
		...
		///@{
		mpu_accel_fsr_t accel_fsr;	 ///< accelerometer full scale range, default ACCEL_FSR_8G
		mpu_gyro_fsr_t gyro_fsr;	 ///< gyroscope full scale range, default GYRO_FSR_2000DPS
		mpu_accel_dlpf_t accel_dlpf; ///< internal low pass filter cutoff, default ACCEL_DLPF_184
		mpu_gyro_dlpf_t gyro_dlpf;	 ///< internal low pass filter cutoff, default GYRO_DLPF_184
		...
		///@{
		unsigned short dmp_sample_rate; ///< sample rate in hertz, 200,100,50,40,25,20,10,8,5,4
		...
		mpu_orientation_t orient;		///< DMP orientation matrix, see mpu_orientation_t
	...
	extern void (*dmp_callback_func)(void);
	...
	/**
	 * @brief      Returns an mpu_config_t struct with default settings.
	 *
	 * Use this as a starting point and modify as you wish.
	 *
	 * @return     Returns an mpu_config_t struct with default settings.
	 */
	 ...
	 *
	 * Only call this after powering on the MPU with mpu_initialize or
	 * mpu_initialize_dmp. This should geenrally be called at the end of your
	 ...
	 *
	 * First create an instance of the mpu_data_t struct and pass its pointer to
	 * mpu_initialize which will then write to. Also pass an mpu_config_t
	 * struct with your configruation settings.
	 *
	 * This function will populate the  accel_to_ms2 and gyro_to_deg fields of the
	 * mpu_data_t struct appropriately based on the user-configured full scale
	 * ranges.
	 *
	 * After this, you may read sensor data at any time with the functions
	 * mpu_read_accel(), mpu_read_gyro(), and mpu_read_temp(). The magentometer
	 * can also be read with mpu_read_mag() if using an MPU9150 or MPU9250 and the
	 * enable_magnetometer field in the mpu_config_t struct has been set to 1.
	 *
	 ...
	 * been set in the user's mpu_config_t when it was passed to
	 * mpu_initialize()
	 *
	 * @param      data  Pointer to user's data struct where new data will be
	 * written
	...
	/**
	 * @brief      Initializes the MPU in DMP mode, see test_dmp example
	 *
	 * After calling this the user does not need to call the normal read functions
	 * mpu_read_accel(), mpu_read_gyro(), or mpu_read mag(). Instead the
	 ...
	 * down due to mpu_power_off, or -1 on error.
	 */
	 ...
	 * yet.
	 */
	int64_t mpu_nanos_since_last_dmp_interrupt(void);
	 ...
	 * @return     0 on success or -1 on failure.
	 */
	int mpu_set_tap_callback(void (*func)(int direction, int counter));
	 ...
	 * due to mpu_power_off(), or -1 on error.
	 */
	 ...
	 * yet.
	 */
	int64_t mpu_nanos_since_last_tap(void);
	 ...
	 * calibrate_gyro example program.
	 *
	 ...
	 * calibrate_accel example program.
	 *
	 * @param[in]  conf  Config struct, only used to configure i2c bus and address.
	 *
	 * @return     0 on success, -1 on failure
	 ...
	 */
	int mpu_is_gyro_calibrated(void);
	 ...
	 * @return     1 if calibrated, 0 if not
	 */
	int mpu_is_mag_calibrated(void);
	 ...
	 * @return     1 if calibrated, 0 if not
	 */
	int mpu_is_accel_calibrated(void);
