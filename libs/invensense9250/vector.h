 ...
 *
 * The small cli_printf(vector_t struct contains information about the vector's size and
 * a pointer to where dynamically allocated memory exists that stores the actual
 * data for the vector. Use vector_alloc to dynamically allocate memory for
 * each new vector. Then use vector_free() and to free the memory when you
...
#define RC_VECTOR_INITIALIZER \
    ...
    /**
 * @brief      Returns an vector_t with no allocated memory and the
 ...
 * with this function before using vector_alloc or any other function.
 *
 * @return     vector_t with no allocated memory and the initialized flag set
 * to 0.
 */
 ...
 * as it is allocated with malloc. Use vector_zeros or vector_ones if you
 ...
 *
 * @param      v       Pointer to user's vector_t struct
 ...
 *
 * Also sets the length and initialized flag of the vector_t struct to 0 to
 * indicate to other functions that v no longer points to allocated memory and
 * cannot be used until more memory is allocated such as with vector_alloc or
 * vector_zeros. Returns 0 on success. Will only fail and return -1 if it is
 ...
 * leaks. It is not necessary to call alloc_vector before this.
 *
 * @param      v       Pointer to user's vector_t struct
 * @param[in]  length  Length of vector to allocate memory for
 *
 ...
 *
 * Any existing memory allocated for v is freed if necessary to avoid memory
 * leaks. It is not necessary to call alloc_vector before this. This is
 ...
 *
 * Simply making a copy of an vector_t struct is not sufficient as the
 * vector_t struct simply contains a pointer to the memory allocated to
 * contain the contents of the vector. vector_duplicate sets b to be a new
 * vector_t with a pointer to freshly-allocated memory.
 *
 ...
 * sign. We recommend vector_print_sci() for very small or very large numbers
 * where scientific notation would be more appropriate.
 *
 * @param[in]  v     Pointer to user's vector_t struct
 *
 ...
 * @brief      Prints to stdout the contents of vector v in one line.
 *
 * Like vector_print but prints with scientific notation. This is not
 ...
 * passing an vector_t struct or matrix_struct as a pointer when its data
 ...
 * infinity and -infinity norms see vector_max and vector_min
 *
