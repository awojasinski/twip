...
// current tolerance, can be changed with algebra_set_zero_tolerance.
double zero_tolerance = DEFAULT_ZERO_TOLERANCE;

// used by QR decomposition
static int __householder_reflection(int step, matrix_t *Q, matrix_t *R)
{
    int i, j, k;
...
int algebra_qr_decomp(matrix_t A, matrix_t *Q, matrix_t *R)
{
    int i, steps;
    ...
    {
        cli_printf("ERROR in algebra_qr_decomp, matrix not initialized yet\n");
        return -1;
    }
    // start R as A
    if (unlikely(matrix_duplicate(A, R)))
    {
        cli_printf("ERROR in algebra_qr_decomp, failed to duplicate A\n");
        return -1;
    }
    // start R as square identity
    matrix_identity(Q, A.rows);
    // find out how many householder reflections are necessary
    if (A.rows == A.cols)
        steps = A.cols - 1; // square
    ...
    for (i = 0; i < steps; i++)
    {
        if (__householder_reflection(i, Q, R) == -1)
            return -1;
    }
    return 0;
}

int algebra_lup_decomp(matrix_t A, matrix_t *L, matrix_t *U, matrix_t *P)
{
    int i, j, k, m, index, tmpint;
    double s1, s2;
    int *ptmp;
    void *rowtmp;
    matrix_t Adup = RC_MATRIX_INITIALIZER;
    // sanity checks
    if (unlikely(!A.initialized))
    {
        cli_printf("ERROR in algebra_lup_decomp, matrix not initialized yet\n");
        return -1;
    }
    if (unlikely(A.cols != A.rows))
    {
        cli_printf("ERROR in algebra_lup_decomp, matrix is not square\n");
        return -1;
    }
    // allocate some memory!
    m = A.cols;
    if (unlikely(matrix_duplicate(A, &Adup)))
    {
        cli_printf("ERROR in algebra_lup_decomp, failed to duplicate A\n");
        return -1;
    }
    if (unlikely(matrix_identity(L, m)))
    {
        cli_printf("ERROR in algebra_lup_decomp, failed to allocate identity matrix\n");
        matrix_free(&Adup);
        return -1;
    }
    if (unlikely(matrix_alloc(U, m, m)))
    {
        cli_printf("ERROR in algebra_lup_decomp, failed to allocate U\n");
        matrix_free(&Adup);
        matrix_free(L);
        return -1;
    }
    if (unlikely(matrix_zeros(P, m, m)))
    {
        cli_printf("ERROR in algebra_lup_decomp, failed to allocate matrix of zeros\n");
        matrix_free(&Adup);
        matrix_free(L);
        matrix_free(U);
        return -1;
    ...
    {
        cli_printf("ERROR in algebra_lup_decomp, alloca failed, stack overflow\n");
        matrix_free(&Adup);
        matrix_free(L);
        matrix_free(U);
        matrix_free(P);
    ...
    for (i = 0; i < m - 1; i++)
    {
        ...
        for (j = i; j < m; j++)
        {
            if (fabs(A.d[j][i]) >= fabs(A.d[index][i]))
                index = j;
        }
        if (index != i)
        {
    ...
    for (i = 0; i < m; i++)
    {
        for (j = 0; j < m; j++)
        {
            ...
            for (k = 0; k < j; k++)
                s2 += U->d[k][j] * L->d[i][k];
            if (j >= i)
                U->d[i][j] = (Adup.d[i][j] - s1);
            if (i >= j)
                L->d[i][j] = (Adup.d[i][j] - s2) / U->d[j][j];
        }
    }
    matrix_free(&Adup);
    return 0;
...
int algebra_invert_matrix(matrix_t A, matrix_t *Ainv)
{
    int i, j, k;
    matrix_t L = RC_MATRIX_INITIALIZER;
    matrix_t U = RC_MATRIX_INITIALIZER;
    matrix_t P = RC_MATRIX_INITIALIZER;
    matrix_t D = RC_MATRIX_INITIALIZER;
    matrix_t tmp = RC_MATRIX_INITIALIZER;
    // sanity checks
    if (unlikely(!A.initialized))
    {
        cli_printf("ERROR in matrix_inverse, matrix uninitialized\n");
        return -1;
    }
    if (unlikely(A.cols != A.rows))
    {
        cli_printf("ERROR in matrix_inverse, nonsquare matrix\n");
        return -1;
    }
    if (fabs(matrix_determinant(A)) < zero_tolerance)
    {
        cli_printf("ERROR in matrix_inverse, matrix is singular\n");
        return -1;
    }
    // allocate memory
    if (unlikely(matrix_identity(&D, A.cols)))
    {
        cli_printf("ERROR in matrix_inverse, failed to alloc identity\n");
        return -1;
    }
    if (unlikely(matrix_alloc(&tmp, A.rows, A.rows)))
    {
        cli_printf("ERROR in matrix_inverse, failed to alloc matrix\n");
        matrix_free(&D);
        return -1;
    }
    // do LUP
    if (unlikely(algebra_lup_decomp(A, &L, &U, &P)))
    {
        cli_printf("ERROR in matrix_inverse, failed to LUP decomp\n");
        matrix_free(&D);
        matrix_free(&tmp);
        return -1;
    }
    ...
    if (unlikely(matrix_multiply(tmp, P, Ainv)))
    {
        cli_printf("ERROR in matrix_inverse, failed to multiply matrix\n");
        i = -1;
    }
    // free allocation
    matrix_free(&tmp);
...
int algebra_invert_matrix_inplace(matrix_t *A)
{
    matrix_t Atmp = RC_MATRIX_INITIALIZER;
    if (unlikely(algebra_invert_matrix(*A, &Atmp)))
    {
        cli_printf("ERROR in algebra_invert_matrix_inplace, failed to invert\n");
        return -1;
    }
...
int algebra_lin_system_solve(matrix_t A, vector_t b, vector_t *x)
{
	...
	*/
    ...
    int nDim, i, j, k, m;
    matrix_t Atemp = RC_MATRIX_INITIALIZER;
    vector_t btemp = RC_VECTOR_INITIALIZER;
    // sanity checks
    if (!A.initialized || !b.initialized)
    {
        cli_printf("ERROR in algebra_lin_system_solve, matrix or vector uninitialized\n");
        return -1;
    }
    if (A.cols != b.len)
    {
        cli_printf("ERROR in algebra_lin_system_solve, dimension mismatch\n");
        return -1;
    }
    // alloc memory for x
    nDim = A.cols;
    if (unlikely(vector_alloc(x, nDim)))
    {
        cli_printf("ERROR in algebra_lin_system_solve, failed to alloc vector\n");
        return -1;
    }
    // duplicate user arguments so we don't have to modify them
    if (unlikely(matrix_duplicate(A, &Atemp)))
    {
        cli_printf("ERROR in algebra_lin_system_solve, failed to duplicate matrix\n");
        vector_free(x);
        return -1;
    }
    if (unlikely(vector_duplicate(b, &btemp)))
    {
        cli_printf("ERROR in algebra_lin_system_solve, failed to duplicate vector\n");
        vector_free(x);
        matrix_free(&Atemp);
        return -1;
    }
    ...
    { // base row of matrix
        ...
        {
            cli_printf("ERROR in algebra_lin_system_solve, matrix not full rank\n");
            matrix_free(&Atemp);
            vector_free(&btemp);
            vector_free(x);
            return -1;
...
void algebra_set_zero_tolerance(double tol)
{
    zero_tolerance = tol;
    return;
}

int algebra_lin_system_solve_qr(matrix_t A, vector_t b, vector_t *x)
{
    int i, k;
    vector_t temp = RC_VECTOR_INITIALIZER;
    matrix_t Q = RC_MATRIX_INITIALIZER;
    matrix_t R = RC_MATRIX_INITIALIZER;
    if (unlikely(!A.initialized || !b.initialized))
    {
        cli_printf("ERROR in algebra_lin_system_solve_qr, matrix or vector uninitialized\n");
        return -1;
    }
    // do QR decomposition
    if (unlikely(algebra_qr_decomp(A, &Q, &R)))
    {
        cli_printf("ERROR in algebra_lin_system_solve_qr, failed to perform QR decomp\n");
        return -1;
    }
    // Ax=b
    // QRx=b
    ...
    if (unlikely(matrix_row_vec_times_matrix(b, Q, &temp)))
    {
        cli_printf("ERROR in algebra_lin_system_solve_qr, failed to multiply vec by matrix\n");
        matrix_free(&Q);
        matrix_free(&R);
        return -1;
    }
    // allocate memory for the output x
    if (unlikely(vector_alloc(x, R.cols)))
    {
        cli_printf("ERROR in algebra_lin_system_solve_qr, failed to alloc vector\n");
        matrix_free(&Q);
        matrix_free(&R);
        vector_free(&temp);
        return -1;
...
int algebra_fit_ellipsoid(matrix_t pts, vector_t *ctr, vector_t *lens)
{
    int i, p;
    matrix_t A = RC_MATRIX_INITIALIZER;
    vector_t b = RC_VECTOR_INITIALIZER;
    vector_t f = RC_VECTOR_INITIALIZER;
    // sanity checks
    if (unlikely(!pts.initialized))
    {
        cli_printf("ERROR in fit_ellipsoid, matrix not initialized\n");
        return -1;
    }
    if (unlikely(pts.cols != 3))
    {
        cli_printf("ERROR in fit_ellipsoid, matrix pts must have 3 columns\n");
        return -1;
    }
    p = pts.rows;
    if (p < 6)
    {
        cli_printf("ERROR in fit_ellipsoid, matrix pts must have at least 6 rows\n");
        return -1;
    }
    // allocate memory for linear system
    if (unlikely(vector_ones(&b, p)))
    {
        cli_printf("ERROR in fit_ellipsoid, failed to alloc vector\n");
        return -1;
    }
    if (unlikely(matrix_alloc(&A, p, 6)))
    {
        cli_printf("ERROR in fit_ellipsoid, failed to alloc matrix\n");
        vector_free(&b);
        return -1;
    }
    // fill in A for QR
    ...
    if (unlikely(algebra_lin_system_solve_qr(A, b, &f)))
    {
        cli_printf("ERROR in fit_ellipsoid, failed to solve QR\n");
        matrix_free(&A);
        vector_free(&b);
        vector_free(&f);
        return -1;
    ...
    // compute center
    if (unlikely(vector_alloc(ctr, 3)))
    {
        cli_printf("ERROR in fit_ellipsoid, failed to allocate ctr\n");
        vector_free(&f);
        return -1;
    }
    ...
    // Solve for lengths
    if (unlikely(vector_alloc(&b, 3)))
    {
        cli_printf("ERROR in fit_ellipsoid, failed to alloc vector\n");
        return -1;
    }
    if (unlikely(matrix_alloc(&A, 3, 3)))
    {
        cli_printf("ERROR in fit_ellipsoid, failed to alloc matrix\n");
        vector_free(&b);
        return -1;
    ...
    b.d[2] = f.d[4];
    // solve for lengths
    if (unlikely(algebra_lin_system_solve(A, b, lens)))
    {
        cli_printf("ERROR in fit_ellipsoid, failed to solve linear system\n");
        matrix_free(&A);
        vector_free(&b);
        vector_free(&f);
        return -1;
